...
var generateUSymbol = module.exports = function generateUSymbol (name, salt) {
  if (!salt && salt !== 0) {
...
module.exports.uSymbolFactory = function uSymbolFactory (name) {
  if (process.env.NODE_ENV !== 'production') {
    assertName(name);
  }

  return function (salt) {
    return generateUSymbol(name, salt);
  };
};
